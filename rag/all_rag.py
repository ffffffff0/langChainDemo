...
def create_embeddings_v2(text, model="text-embedding-3-large"):
...
def context_enriched_search(query, text_chunks, embeddings, k=1, context_size=1):
    query_embedding = create_embeddings_v2(query).data[0].embedding
    similarity_scores = []
...
with open('../example_data/q_a.json') as f:
    data = json.load(f)
...
def generate_response(query, system_prompt, retrieved_chunks, model="gpt-4o"):
